...
class Service(object):
    ...
    def find_kudo(self, repo_id):
        kudo = self.repo_client.find({'user_id': self.user_id, 'repo_id': +
                                      repo_id})
    ...
    def update_kudo_with(self, repo_id, githubRepo):
        records_affected = self.repo_client.update({'user_id': self.user_id, +
                                                    'repo_id': repo_id},   self.prepare_kudo(githubRepo))
